		...
		<div class="container flex items-center justify-between gap-4 py-2">
			...
			<nav
				...
				<ul class="flex items-center gap-8 max-md:flex-col">
					<li v-for="route in visibleRoutesList" :key="route.path" class="item">
						<router-link
							class="text-2xl capitalize md:text-lg font-medium hover:text-primary transition-colors duration-300"
			...
			<div class="flex items-center gap-2">
				<button
					type="button"
					class="w-10 h-10 flex focus-visible:outline-none items-center justify-center enabled:hover:bg-primary-500/15 rounded-full transition-all text-surface-900 dark:text-surface-0 focus-visible:ring-1 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-surface-0 dark:focus-visible:ring-offset-surface-950 z-10 border border-primary-200 dark:border-primary-700"
...
const visibleRoutesList = computed(() => routes.filter((r) => r.meta.showInNavMenu))
</script>
